...
Logistic regression is a type of generalized linear model (GLiM) that uses statistical analysis to predict an event based on known factors. I use logistic regression here to make predictions about whether a song will be popular based on features we used before like tempo, valence, danceability and energy ect.

At first, I change some factors into categorical that can be better use for further analysis.
...
data1960Logit_4$fitted.values[1]
predict(data1960Logit_4)[1]
1/(1+exp(-predict(data1960Logit_4)[1]))
```
...
p-value is less than 0.05, that we can reject the null hypothesis.
...
```{r roc_auc}
loadPkg('pROC') 
prob=predict(data1960Logit_4, type = c("response"))
data1960Logit_4$prob=prob
...
mcFadden = 1 - logLik(data1960Logit_4)/logLik(data1960NullLogit)
mcFadden
```
...
data1960Logitpr2 = pR2(data1960Logit_4)
data1960Logitpr2
unloadPkg('pscl') 
```
...
I do the same thing in year 2000. The same method can be good for comparing between 1960s and 2000s.
...
There are a lot of factors that can affect the song's popularity in different years. From the logistic regression, we can draw conclusions from our dataset below.

In the year 1960s if a song wants to be popular, it needs to have features of energy, danceability, valence and tempo. In 2000s, however, if a song wants to be popular, it needs to have features of danceability, energy and liveness.
