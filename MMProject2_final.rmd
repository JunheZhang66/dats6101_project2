...
From the three models explored above using popular and non-popular songs from the 1960's and 2000's decades and evaluating the musical attributes from therein, the answer to the SMART quesiton posed is **'Yes'**.  All three models had an accuracy significantly greater than 50%, fluctuating from 64% from the Logistic Regression modeling to 75% from the K-Nearest Neighbor models, identifying that, *'Yes'*, musical attributes can be used to identify popular songs from non-popular songs, at least to some extent or at a given, non-50-50 probability (if they could not, the accuracy for each model would approximately be 50%), and, *'Yes'*, these attributes can also change with time although there are some consistent attributes of popular songs through time.

In both of the evaluated decades, **danceability**, **energy**, and **key** were consistently significant features in determining song popularity, while **liveness**, **tempo**. and **valence** were only significant in a single decade.  *Key* was the only musical attribute that was determined to not have an effect on song popularity; this is consistent with the exploratory data analysis of the variable showing no distinct difference in the mean value of key or distribution of the feature between popular and non-popular songs.

The conclusion of this study, for the benefit of any aspiring song writer who wishes to produce popular, well-respected music, is to create songs that have a high energy, are easy to dance to, and are played in the major key, so as to evoke an uplifting mood in listeners as these attributes more likely to producing a popular song.  Additional analysis, potentially reviewing lyrics, themes, or other musical attributes, may lead to additional insights into the structure and composition of popular songs to the benefit of musical scholars, artists, or profiteers.
...
The project goal is to explore which features determine whether a song is popular or not. K-Nearest Neighbor is a good option to apply on this dataset. Since K-Nearest Neighbor algorithms use distance function to measure distance between each data point, verifying that every predictor has same unit is very important. 

From the previous EDA section it showed that **tempo, key, and mode** have much greater mean values than other predictors; therefore, in order to have the highest accuracy, a K-Nearest Neighbor model scaling the data using Z-score is necessary.  Additionally, since the K-Nearest Neighbor algorithm highly depends on distance function, cleaning the categorical features like **track and artist** is also very important.
 
From the scaled dataset, notice that **tempo** for 2000 data is smaller than 1960 data in unit factor, the **mode and valence** value range for 2000 data are more spread than 1960 data showing that music in the 2000's have more varieties than music in 1960. However, the 2000's music, in general, has more **energy** than 1960's musics. 
...
Before building KNN model, in order to preventing high collinearity between each predictor, it is also necessary to check the correlation between each predictor. From the correlation matrix for 2000s dataset and 1960s dataset below, all predictors are not highly related to each other.  
...
Since data is very precious, it is always a good idea to split the original dataset into train and test subsets. Use the training data set to fit a KNN model and use the test dataset to test the accuracy of the model is the best way to utilize the dataset. Split the original data into 4:1 train and test subset.
...
In order to test accuracy of the fitted models, splitting the target variable **target** (1 stands for "popular", 0 stands for "not popular") variable from the predictors is also necessary.
...
This helper function applies the K-Nearest Neighbor model function from the **class** library, then builds a confusion matrix using the predicted target value with the test target value. Finally, the accuracy is calulated using the formula **TP/(TP+NP)** from the fitted KNN model with a given K value (or number of nearest neighbors from which to evaluate). 

Accuracy is a very important factor to determine the goodness of a fitted KNN model. On the other hand, because a high K-value indicates high precision and high cost on computation, carefully determining the appropriate K-value in order to manage the best trade-off between precision and computation cost is very important. In other words, this study will use the exhaustive approach to test a series of K-values and find the most appropriate K-value (where the increase in accuracy from a higher K-value reaches a significant diminshment in returns).
...
In order to build the best KNN model with the given datasets to predict whether a music is popular or not. It is necessary to use an exhaustive approach to build models with all combinations of predictors and decide which combination of predictors will generate the most accurate model.

The **featureSelectionKNN** helper function applies the **combn** function which helps generate a list of **n Choose i**, where n stands for number of predictors, i stands for number of predictors we want to consider in each iteration from i to n. Within each iteration, apply **chooseK** function to determine the best k-value for this combination of predictors. Then store the model with the highest accuracy into the result dataframe.
...
Build the KNN model using 1960s dataset with the best subset of features and check correctness of the above **featureSelectionKNN** result. Notice that the best K-value among 1 through 81 is still 19 which is as same as the result we get from **featureSelectionKNN** function. On the other hand, k-value equal 19 is also the ankle value of the K-values series which is the best K-value we are looking for.
...
## Grow the Tree
In this section, the study will be using the Categorical Decision Tree to show what makes a song popular in the 1960's verses the 2000's.  The variables of tempo, key, valence, danceability, energy, liveness, and mode are the independent variables and target will be used as the dependent variable.  First, there had to be change made to the variable, 'target', so it could be a factor variable.  Once this has been done, one can proceed to grow the decision tree by starting at the root and building up.  Our root will be target which has already been stated as the dependent variable:
...
For the 1960's Song List:
From the decision tree, the first branching point has 4321 popular and 4321 unpopular songs from the data. The first split yields 2015 outcomes with energy levels < 0.27 and 6627 outcomes with energy levels >= 0.27 from the popularity results for the song list.  The energy level was shown as being the most important variable to determine whether the song was popular or not.  The energy level being under 0.27 represents that the song was more likely to be unpopular and the energy level greater than 0.27 deems the song was more likely to be popular.  The variable that follows after the energy level was the variance level.  There were only two variables that determined the popularity of the songs making the Billboard Top 100 list. For the song to more liekly be considered popular, the energy of the song has to be equal to or greater than 0.27.  A further split on the left node might not have been beneficial to separate the popular and unpopular songs list. The algorithm actually stops there, and more likely predicts an unpopular rating for all 2015 observations, giving 476/2015 incorrect predictions. 

Nonetheless, the lower branch continues to split, with  (67%) correct prediction on the last leaf (node at end of a branch) and 481/772 correct **non popular** prediction on the third leaf, while the last leaf predicted 3604/5905 **popular** correctly. Not too bad.
...
Now the 2000's Song List can be summarized:
The decision tree starts the first branching point with 2936 popular and 2936 unpopular songs from the data presented from Spotify. The first split yields 1319 outcomes with danceability levels < 0.396 and 4553 outcomes with danceability levels >= 0.396 for the popularity results for the song list.  The danceability level has been identified as the most important variable to determine whether a song was popular or not in the 2000's.  The danceability level being under 0.396 represents that the song was more likely unpopular while the danceability level being greater than 0.396 are deemed as the song being more-likely popular.  The variables that follow after the danceability level are the energy and then valence levels.  Unlike the 1960's, there are three variables that determined the popularity of the songs making the Billboard Top 100 list. For the song to be more likely considered popular, the danceability of the song has to be equal to or greater than 0.396.  A further split on the left node might not have been beneficial just like in the 1960's decision tree to separate the popular and non-popular songs list. The algorithm actually stops there, and predicts non-popular for all 1319 observations, giving 157/2015 incorrect predictions. 

Nonetheless, the lower branch continues to split, with  (60%) correct prediction on the last leaf (node at end of branch) and 129/214 correct **non-popular** prediction on the third leaf, while the last leaf predicted 609/976 **popular** correctly.

Overall, when the model predicts **unpopular**, accuracy is (1539+481)/(1539+476+481+291) = `r round( (1539+481)/(1539+476+481+291)*100,1 )`%. And when the model predicts **popular**, accuracy is (29+12+12)/(29+0+12+0+12+2) = `r round( (29+12+12)/(29+0+12+0+12+2)*100,1 )`%.

From the decision tables above we see that what made a song popular in the 1960's did not necessarily make a song popular in the 2000's.  They had one differing variable that made the difference.  In the 2000s, the most important variable for the song to make the Billboard's Top 100 list was for the song to have danceability.  A song like 'Hot in Herre' by Nelly has the high danceability that is discussed above.  In the 1960's, a song needed to have energy as the most important variable.  We can check our decision tables by creating confusion matrixes.

These perentages for both the 1960's and 2000's songs can be represented by the calculations below that are being examined in confusion matrixes.
...
The following are two additional ways that we can plot out the categorical decision tree.  The additional methods are the 'rpart.plot' and 'fancy' plots to help read the tree easier.  Both the 1960's and 2000's songs have been placed into the models as follows:


1960's Song Fancy and Rpart Plots
...
2000's Song Fancy and Rpart Plots
...
We can now prune the decision trees to get the most important results to know what variables are most important to be put onto the Billboard's Top 100 list.  The following are from the pruning of the trees:
...
From the pruned trees, the results show different variables for each year on what classified them to get on the Billboard's Top 100 List.  For the 1960's songs, the song needed to have an energy level greater than 0.27.  This is shown as the most significant variable that determines the popularity of a song.  In order for a song in the 2000's to make the list, the danceability matters more than energy.  The dancebility had to be greater than a level of 0.40.  In the two eras, there were differences in what was deemed as most important.  In the 1960's, a person needed to have the right energy being exuded from the music, while in the 2000's the song had to be a danceable song.
...
The purpose of this study was to answer whether musical attributes change between popular, American songs and non-popular songs and with time.  From the three models explored above using popular and non-popular songs from the 1960's and 2000's decades and evaluating the musical attributes from therein, the answer to the SMART quesiton posed is **'Yes'**.  All three models had an accuracy significantly greater than 50%, fluctuating from 64% from the Logistic Regression modeling to 75% from the K-Nearest Neighbor models, identifying that, yes, musical attributes can be used to identify popular songs from non-popular songs, at least to some extent or at a given, non-50-50 probability (if they could not, the accuracy for each model would approximately be 50%).  Following that, the most significant musical attributed from the models (detailing the musical attributes that had the greatest effect in predicting popularity of songs) are summarized below:

*From the 1960's*, increasing the **danceability**, **energy**, **tempo**, and **valence** of a song and playing the song in a **major** key increased the likelihood that the song would be popular.

*From the 2000's*, increasing the **danceability** and **energy** of a song, not playing the song live, and playing the song in a **major** key increased the likelihood that the song would be popular.

In both of the evaluated decades, **danceability**, **energy**, and **key** were consistently significant features in determining song popularity, while **liveness**, **tempo**. and **valence** were only significant in a single decade.  *Key* was the only musical attribute that was determined to not have an effect on song popularity; this is consistent with the exploratory data analysis of the variable showing no distinct difference in the mean value of key or distribution of the feature between popular and non-popular songs.

As the observed models identified different, significant musical attributes between each decade, it can be seen that, in addition to these musical attributes having an effect on the popularity of song, these attributes can also change with time although there are some consistent attributes of popular songs through time.
