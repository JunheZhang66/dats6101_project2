...
The main goal of this section is to predict whether the target class of popular and non popular songs will have different variables that affect them in the 1960s and 2000s.  Will the songs have the same variables that matter most in both years or will they differ?  We chose a classification decision tree to be able to assign the classification of popular or non popular with various variables.

## Grow the Tree
In this section, the study will be using the Categorical Decision Tree to show what makes a song popular in the 1960s verses the 2000s.  The variables of tempo, key, valence, danceability, energy, liveness, and mode are the independent variables while target will be used as the dependent variable.  First there had to be changes made to the variable target so it could be a factor variable.  The label of 0 will mean that the song is unpopular and the label of 1 will stand for the song being popular.  Once the change oocurs, one can proceed to grow the decision tree by starting at the root and building up.  Our root will be target which has already been stated as the dependent variable of target:
...
set.seed(1)
data1960_DT <- rpart(target ~ tempo + key + valence + danceability + energy + liveness + mode, data=data1960, method="class", control = list(maxdepth = 10) )
...
set.seed(1)
data2000_DT <- rpart(target ~ tempo + key + valence + danceability + energy + liveness + mode, data=data2000, method="class", control = list(maxdepth = 10) )
...
Due to the complexity of reading the above trees, we can find an easier way.  The following trees below are an alternative way that allows one to be able to plot out the categorical decision tree.  The alternative methods are the 'rpart.plot' and 'fancy' plots to help read and display the tree easier.  Both the 1960s and 2000s songs have been placed into the models as follows:
...
loadPkg("rattle") # For fancyRpartPlot (Trees) Answer "no" on installing from binay source
...
The classification trees show how the root node which has an even number of popular and non popular songs is grown to reach a leaf (terminal) node.  The root node has the question of is the song popular or not via the chosen variable.  We have broken up the two trees below:

The 1960s Song List Classification Tree:
From the decision tree, the first branching point has 4321 popular and 4321 unpopular songs from the Spotify data. The root question which starts the decision tree of: Is the song popular or not?  The first split has the variable on energy which shows that the energy level < 0.27 had 2015 outcomes and 6627 outcomes showed that the energy levels >= 0.27 but has a higher percentage for popular songs.  The energy level being under 0.27 represents that the song was more likely to be unpopular and the energy level greater than or equal to 0.27 deems the song as likely to be more popular.  The variable that follows after the energy level was the mode level.  A further split on the left node might not have been beneficial to separate the popular and unpopular songs list. The algorithm on the left side stops there with the energy being below 0.27.  The predictions for unpopular for all 2015 observations gives 463/2015 incorrect predictions. 

Now the 2000s Song List can be summarized as follows:
The decision tree for the 2000s is similar to that of the decision tree of the 1960s.  The root question for this decision tree is the same as the 1960s with the question of: Is this song popular or not?  The very first branching point has 2936 popular and 2936 unpopular songs. The first split has the variable on danceability whihc is different from the 1960s variable of energy.  The danceability level < 0.40 had 1319 outcomes and 4553 outcomes showed that the danceability levels >= 0.40 but has a higher percentage for popular songs.  The danceability level being under 0.40 represents that the song was more likely to be unpopular and the danceabiliyt level greater than or equal to 0.40 deems the song as likely to be more popular.  The variable that follows after the danceability level was a higher level of danceability level.  The danceability level on the second level was whether the danceability level < 0.57.  When the danceability < 0.57, the song has a higher rate of being unpopular while when danceability >= 0.57 there is a greater chance of the song being popular.  A further split on the left node might not have been beneficial to separate the popular and unpopular songs list just like in the 1960s. The algorithm on the left side stops there with the danceability being below 0.40.  The predictions for unpopular for all 1319 observations gives 157/2015 incorrect predictions.   

From the decision trees above we see that what made a song popular in the 1960s did not necessarily make a song popular in the 2000s.  They had one differing variable that made the difference.  In the 2000s, the most important variable for the song to make the Billboards Top 100 list was for the song to have danceability.  Songs like Hot in Herre by Nelly is a song that has the danceability that can be talked about.  In the 1960s, a song needed to have energy as the most impoartant variable.  We can check our decision tables by creating confusion matrixes.

The 1960s and 2000s songs can represented by the calculations below that are being examined in the confusion matrixes.


## Confusion Matrixes

The confusion matrix, also known as the error matrix, helps to describe the performace of a classification model and identify confusion between classes.  From the confusion matrix, the summary of prediction results can be calculated on the classification trees above.  For this confusion matrix, 0 will be for unpopular songs while 1 will be for the popular songs.  The matrix trees for both the 1960s and 2000s are as follows:

### 1960s Confusion Matrix
...
From the matrix confusion tables, the following can be concluded:

1960s Song Overall Accuracy = `r round(cm_1960$overall["Accuracy"]*100, digits=2)`%
2000s Song Overall Accuracy = `r round(cm_2000$overall["Accuracy"]*100, digits=2)`%
The accruracies for both decades are rather high indicating a positive chance of getting a truly popular song or not.

The overall recall for the 1960s is `r round(cm_1960$byclass["Recall"]*100, digits=2)`% and the 2000s is `r round(cm_2000$byclass["Recall"]*100, digits=2)`%.  The recall results for the 1960s is close to half and has rise to have suspicion.  Due to it being higher than 50%, there is a better chance that the popular songs are truly popular.  The recall rate for the 2000s also backs that there is a good possibility that the songs are truly popular when picked.  

In the case of the precision, in the 1960s the precison is show to be `r round(cm_1960$byclass["Precision"]*100, digits=2)`% and the 2000s is `r round(cm_2000$byclass["Precision"]*100, digits=2)`%. Both years show a high precision.  Therefore we can conclude that the possibility of the songs being truly popular or unpopular is of a good chance.
...
for (deep in 2:6) {
  fit1960 <- rpart(target ~ tempo + key + valence + danceability + energy + liveness + mode, data=data1960, method="class", control = list(maxdepth = deep) )
  # 
...
for (deep in 2:6) {
  fit2000 <- rpart(target ~ tempo + key + valence + danceability + energy + liveness + mode, data=data2000, method="class", control = list(maxdepth = deep) )
  # 
...
Pruning the tree is a method that reduces the original decision down to the most bare sections that have the most power in determining which classifiers are most important.  Therefore the method was used for both the 1960s and 2000s decision trees to identify what the most important variables were to concluded that a song could make it onto the Billboards Top 100 List.  The following shows the pruning of each tree:
...
### Pruned Tree Conclusion
From the pruned trees, the results show different variables for each year on what classified them to get on the Billboards Top 100 List.  For the 1960s songs, the song needed to have an energy level greater than 0.27.  This is shown as the most independent variable that determines the popularity of a song.  In order for a song in the 2000s to make the list, the danceability matter more than energy.  The dancebility had to be greater than a level of 0.40.  In the two eras, there were differences in what was deemed as most important.  Using the classification decsion tree method shows that in the 1960s a song needed to have the right energy being eluded from the music while in the 2000s the song had to had a strong danceability level to it.
